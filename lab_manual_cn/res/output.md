...
本实验的目标是使用标准的静态与动态分析工具对 C 程序进行分析，以发现除零错误，并解读分析结果，从而更好地理解不同技术之间的权衡。具体来说，我们将使用 <a href="https://github.com/google/AFL">AFL</a>（American Fuzzy Lop，一种动态分析器/模糊测试器）和 <a href="https://clang-analyzer.llvm.org/">CSA</a>（Clang Static Analyzer，一种静态分析器）。

## 前置要求

课程讲座中介绍了本实验使用的各种术语，例如：静态分析与动态分析、完备性、可靠性、精确率、召回率等。
...
如果你不熟悉 `CMake`，我们建议阅读 [CMake 教程][cmake-tutorial]（尤其注意教程中的步骤 1 和步骤 2）。
...
我们不期望每个人报告相同的解决方案，因为 AFL 本身是非确定性的。
...
确定 C 程序关于除零错误的真实情况（`正确` vs. `错误`）。
具体来说，对于每个程序中的每个除法指令，通过检查程序来确定是否存在某些程序输入会导致除零错误。
将你的答案填写在文件 `lab1/answers.txt` 中每个测试的表格的“真实情况”列。

### 步骤 3.

研究 AFL 和 CSA 的输出，确定它们是否接受或拒绝每个程序。
将你的答案填写在文件 `lab1/answers.txt` 中每个测试程序的表格的相应列中。

AFL 发现的导致崩溃的输入存储在目录 `lab1/results/afl_logs/<test-name>/afl_output/crashes/` 下的单独文件中。
这些文件具有独特的名称，形式如 `id:000000,sig:08,src:000000,op:arith8,pos:2,val:-8`。[^1]。
这些文件的内容就是 AFL 在遇到崩溃时用作测试程序输入的。
...
使用你在步骤 2 和步骤 3 中填写的内容，计算每一列的精确率、召回率和 F1 分数。
...
+ 理解一种名为 [LLVM IR][llvm-lang] 的 C 程序表示形式，我们将在实验中使用它。它是 [LLVM][llvm]（一个流行的多语言编译器框架）使用的中间表示。
+ 通过使用 [LLVM API][llvm-api] 编写一个 [LLVM pass][llvm-pass] 并运行它来静态地找出程序中的所有二元运算符并对它们进行插桩，从而理解该 API。
+ 通过执行插桩后的代码，理解程序的静态性质与动态性质之间的区别。

## 前置要求

+ 阅读关于 LLVM 入门：第一部分（LLVM 概述）和第二部分（LLVM IR 结构）的课程幻灯片。这是完成本实验第一部分以及后续课程中为了调试目的而阅读 LLVM IR 所必需的。
...
+ 确保 Docker 在你的机器上运行。
+ 按 F1 打开 VS Code [命令面板][command-palette]；搜索并选择 `Reopen in Container`。
+ 这将在 VS Code 中为本实验设置好开发环境。
+ 在开发环境中，Lab 2 的骨架代码位于 `/lab2` 目录下。
...
学习 LLVM 入门指南，理解 LLVM IR 的结构。该指南展示了如何在一个示例 C 程序上运行 `clang` 来生成相应的 LLVM IR 程序。你可以使用 `/lab2/test` 目录下的 C 程序来尝试：
...
通过填写 `/lab2/c_programs` 目录下提供的模板代码，手动编写与 `/lab2/ir_programs` 目录下的 LLVM IR 程序相对应的 C 程序。确保在你手写的 C 程序上运行上述命令，能够生成与提供的完全相同的 LLVM IR 程序，因为我们将进行自动评分。你可以使用 diff[^1] 命令行工具来检查你的文件是否相同。
...
在本实验及未来的实验中，我们将使用 `CMake`，这是一个用于管理构建过程的现代工具。如果你不熟悉 `CMake`，强烈建议你先阅读 [CMake 教程][cmake-tutorial]（特别是教程中的步骤 1 和步骤 2）。运行 `cmake` 会生成一个你可能更熟悉的 Makefile。如果不熟悉，请先阅读 [Makefile 教程][makefile-tutorial]再继续。*一旦生成了 Makefile，编辑源文件后，你只需要调用 `make` 来重新构建你的项目。*运行以下命令来设置本实验的这一部分：
...
如步骤 1 所述，你将把本实验的功能实现为两个 LLVM pass，分别称为 `StaticAnalysisPass` 和 `DynamicAnalysisPass`。LLVM passes 是 LLVM 框架的子进程。它们通常对程序执行转换、优化或分析。每个 pass 都作用于输入程序的 LLVM IR 表示。因此，要在输入 C 程序上运行本实验，你必须首先像在第一部分中那样将程序编译为 LLVM IR：
...
`opt` 是一个 LLVM 工具，用于对 LLVM IR 执行分析和优化。选项 `-load-pass-plugin` 加载我们的 LLVM pass 库，而 `-passes='function(static-analysis)'` 指示 opt 在 `simple0.ll` 上运行该 pass。（库可以并且通常包含多个 LLVM passes。）查阅 [opt 的文档][opt-doc] 以了解使用该工具的可能方式；它可能有助于你构建和调试解决方案。类似地，我们使用 `opt` 在编译后的 C 程序上运行提供的 `DynamicAnalysisPass`：
...
`simple0.static.ll` 中产生的程序应与 `simple0.ll` 相同，而 `simple0.dynamic.ll` 中的程序在本实验中则不会相同。你可以使用 `diff`[^1] 来验证这一点：
...
如前所述，我们提供了 `src/StaticAnalysisPass.cpp`，其中包含一个静态分析，用于报告程序中所有指令的位置，你将向其中添加另一个分析。首先花些时间理解提供的分析，它打印出所有指令的位置；LLVM 入门指南将有助于理解这里使用的 API。接下来，你将实现一个静态分析，打印出每个类型为 BinaryOperator 的指令的种类、位置和操作数，并按照以下格式打印：
...
它涉及检查正在运行的程序以获取其运行时状态和行为的信息；这与静态分析（分析代码独立于任何执行的属性）形成对比。检查程序运行时行为的一种方法是在编译时将代码注入程序；这种技术属于[插桩][instrumentation-def]的范畴。对于 `src/StaticAnalysisPass.cpp` 中的每个静态分析，我们将在 `src/DynamicAnalysisPass.cpp` 中有一个相应的动态分析插桩。我们为你提供了第一个分析的实现，它在每条指令之前注入对 `__coverage__` 函数的调用，该函数将正在执行的指令的行号和列号存储到覆盖率文件中。研究该实现以理解用于注入函数的 API。你将实现一个动态分析，跟踪二元运算符的种类、位置以及操作数的运行时值。为此，你需要检查一条指令是否是 `BinaryOperator`，并使用 `instrumentBinOpOperands` 函数对其进行插桩，接下来你将实现这个函数。`instrumentBinOpOperands` 函数必须在每个二元运算符之前注入对 `__binop_op__` 的调用。你可以看到 `__binop_op__` 接受 5 个参数，即：运算符的符号、操作的行号和列号以及两个操作数的运行时值。你可以使用 `getBinOpSymbol` 函数来获取对应于该运算符的符号。为了获取操作数的运行时值，需要记住在 LLVM 中，**由指令定义的变量由指令本身表示**。
...
我们提供了一个框架，你可以在此基础上构建你的 LLVM pass。你需要编辑 `src/DynamicAnalysisPass.cpp` 文件，为你的 LLVM Pass 实现功能。文件 `lib/runtime.c` 包含你将使用你的 pass 注入的函数：
...
在完成第一部分并完成静态分析后，熟悉了 LLVM IR、LLVM 指令和 `Instruction` 类的组织方式后，你就可以开始处理 `DynamicAnalysisPass` 了，为此你需要使用 LLVM API 向程序中插入额外的指令。在 LLVM 中有[多种方法可以做到这一点][llvm-insert-inst]。使用 LLVM 时的一种常见模式是创建一条新指令，并将其直接插入到某条指令的**_前面_**。例如，考虑以下代码片段：
...
创建了一条新指令 (`NewInst`)，并将其插入到现有指令 `ExistingInstruction` 的_前面_。`Instruction` 的子类有类似的方法来实现这一点。特别是，对于本实验，你可以使用这种模式来创建和插入调用指令 (`CallInst`)，如下所述。你还应该看看在 `instrumentCoverage` 函数中是如何将调用指令插入程序的，作为下面指令的示例。
...
这里，`return_type`、`arg1_type`、... `argN_type` 是描述函数参数的 LLVM Type 的变量。例如，C 类型 `int` 通常是 LLVM 类型 `i32`，`char` 是 `i8`，`boolean` 是 `i1`。这一步类似于在 C 或 C++ 中声明函数。

接下来，假设你希望该函数在某条指令 I 之前被调用。为此，你需要使用 [CallInst::Create][callinst-create] 创建一个调用指令，如下所示：
...
这里，你应该用适当的函数参数值填充 `std::vector<Value *> Args`。此外，如前所述，在 LLVM 中，由指令定义的变量由指令本身表示。此外，`Instruction` 类是 `Value` 的子类；这使得将由 Instruction 定义的变量作为参数传递给函数变得相对简单。
...
你需要收集这些信息并将其转发给适当的函数。并非每条 LLVM 指令都对应其 C 源代码中的特定行。因此，在使用调试信息之前，你通常需要检查一条指令是否确实拥有它。

## 理解代码的静态和动态性质

代码有两种类型的性质：静态和动态。静态性质是可以从代码的源代码表示中推断出来的，并且独立于程序的任何特定运行。另一方面，代码在运行时的行为由其动态性质捕获。在第二部分中，你实现了一个 LLVM pass，它静态地找出所有二元运算符及其操作数；你还实现了一个 LLVM pass，它对所有二元运算符进行插桩，以收集描述在程序的给定运行中哪些二元运算符被执行、以什么顺序执行以及使用什么操作数执行的动态性质。静态和动态性质都告诉我们关于程序的有趣事实，这些事实可以以各种方式加以利用。特别是，在本课程中，我们将使用它们来查找程序中的错误。
...
模糊测试是一种流行的软件测试技术，它向被测程序输入随机生成的测试用例。
这类输入有助于发现程序中各种与安全相关和导致崩溃的缺陷。
为此，你的模糊测试器将从种子输入开始，通过对之前的输入进行变异来生成新输入。
它将使用前几轮测试的输出作为_反馈_，来指导后续的测试生成。
你将使用在 `lab2` 中看到的代码覆盖率指标，来帮助选择值得变异的_有趣_输入。

## 环境配置

Lab3 的代码位于 `/lab3` 目录下。
在描述文件位置时，我们会经常将 Lab 3 的顶级目录称为 `lab3`。
请在 VSCode 中打开 `lab3` 目录。

本实验基于 `lab2` 的代码覆盖率插桩技术。
...
模糊测试器和插桩工具使用 CMake 构建，你可以运行以下命令来构建它们：
...
`fuzzer` 是一个工具，它会将（你生成的）随机化输入提供给一个经过插桩的 C 程序，
该程序在遇到除零错误时会优雅退出，并在执行过程中报告代码覆盖率。

### 步骤 2.

接下来，我们需要准备一个测试程序，用 `fuzzer` 对其进行模糊测试。
这需要先对程序进行插桩，类似于 `lab2` 的操作。
...
你可能还记得在 `lab1` 中，AFL 可以永远生成新输入而不会停止运行。
你的模糊测试器也是如此。
因此，我们将使用 `timeout` 命令在指定时间后停止模糊测试器。

之后，你可以运行模糊测试器对 sanity 程序进行 10 秒的测试：
...
这将在 `sanity1` 上运行 `fuzzer` 十秒钟，并将结果存储到 `lab3/test/fuzz_output_sanity1` 目录。
此外，它还会使用 `lab3/config.txt` 来设置 `seed`（用于生成随机数）和 `freq`（决定我们将非崩溃输入写入输出的频率，值越大频率越低）。
由于我们预计会看到更多非崩溃输入，因此使用 `freq` 来控制记录非崩溃输入的频率。

运行 `fuzzer` 后，你应该会看到 `failure` 目录中填充了几个随机生成的、会导致 `sanity1.c` 崩溃的输入。
...
1.  能够生成符合程序输入语法的测试用例，
2.  具有变异测试输入以增加代码覆盖率的策略，
3.  拥有一个反馈机制，帮助驱动所使用的变异类型。
...
现在，假设这个程序是一个黑盒，我们只能通过使用不同输入运行代码来搜索错误。

我们可能会尝试一个随机字符串，比如 `"abcdef"`，这会得到一个成功的运行。
然后，我们可以以第一个字符串为起点，添加一些新字符，比如 `"ghi"`，得到 `"abcdefghi"`。
这里我们变异了原始输入字符串，生成了一个新的测试用例。
我们可能会重复这个过程，最终偶然发现 `"abcdefghijklm"`，它的长度能被 13 整除，从而导致程序崩溃。

第二个情况呢？
我们可以不断在字符串末尾插入字符，最终会得到一个满足 if 语句第一个条件（输入长度大于 100）的大字符串，
但我们还需要执行另一种类型的变异——随机改变字符串中的字符——才能最终满足 if 语句中的第二个条件。
...
我们已经看到随机化测试如何发现错误，以及它如何成为一种有用的软件分析工具。
上一节描述了暴力生成测试用例的方法；我们只是执行随机变异，希望找到错误。
...
你需要实现一些不同的变异函数，变异函数接收一个字符串，对其执行某些变异操作，并返回变异后的字符串。
你必须决定选择哪些变异策略，并在 `selectMutationFn` 中实现你的逻辑。
查看 `src/test/` 中的测试程序可能有助于了解你的模糊测试器需要探索哪些类型的程序才能发现错误，以及你可能想要执行哪些类型的变异。

模糊测试器首先从命令行指定的输入目录中读取输入文件，以初始化 `SeedInputs` 向量。
之后，它需要从 `SeedInputs` 向量中选择一个特定的输入，以及一个用于变异该输入的变异函数。
为此，你需要分别实现 `selectInput` 和 `selectMutationFn` 的逻辑。
一旦模糊测试器选择了一个输入和一个变异函数，它就会对输入进行变异。
变异后的输入将在目标程序上运行，并根据该次运行的覆盖率提供反馈。
利用这个覆盖率，你将决定这是否是一个_有趣_的种子，如果是，则将其插入到 `SeedInput` 向量中。
这使得变异后的输入可以在后续被选中并进一步变异。
...
欢迎尝试其他变异方式，看看是否能加快在二进制程序上搜索错误的速度。
你可以使用 C++ 函数 `rand()` 来生成随机整数。

**注意：** 为了使变异函数更通用，你可以从一个特设版本开始。例如，一个专门插入换行符的版本。最初，这个仅限于添加换行符的函数是特设的。要使其通用化，可以从仅插入换行符过渡到包含一系列随机字符。

你会注意到，不同的程序需要不同的策略，或者在某些情况下，你甚至可能需要在模糊测试过程中切换不同的变异策略。
你需要包含一个机制，该机制会根据覆盖率反馈尝试为输入程序选择最佳策略。
...
5.  将一个有趣的变异体插入到 `SeedInput` 池中，以驱动进一步的变异。
...
在开始之前，请阅读 `Fuzzer.cpp` 文件中的注释、提示和说明，以便更好地理解各个部分是如何组合在一起的。

从小处着手。
一次实现一种变异策略，先尝试让较简单的测试用例崩溃，然后再处理更难的。
一旦成功，你就可以继续实现更多策略，以及基于你获得的反馈在它们之间进行选择的更复杂方法。

不要害怕在模糊测试的轮次之间跟踪任何状态。

你可能想先尝试每种变异策略，看看哪种策略能生成增加代码覆盖率的测试，然后再利用该策略。

### 评分

我们期望你的模糊测试器能够为 `lab3/test` 中提供的所有程序生成崩溃输入。

除此之外，我们还将对十个隐藏的测试程序测试你的模糊测试器。
这些程序对你的模糊测试器来说是更具挑战性的测试用例。
要在隐藏测试中获得满分，你的模糊测试器应该能够在至少七个程序中找到崩溃输入。
...
所有测试用例将使用相同的种子值。如果这样做，你需要提交对 `lab3/config.txt` 的更改。
...
在本实验中，你将应用基于性质的测试，基于 [Hypothesis](https://github.com/HypothesisWorks/hypothesis) 来验证二叉搜索树的实现。Hypothesis 是 Python 中一个流行的基于性质的测试工具。通过本实验，你将学习如何为有效测试定义不同形式的性质。
...
在本实验中，`lab4/src/BST.py` 提供了二叉搜索树的实现。该 BST 实现支持任何可比较类型的键和任何类型的值，以及四个核心操作：`insert`、`delete`、`find` 和 `union`。请仔细阅读代码以理解其实现。

### Hypothesis

Hypothesis 是一个实现基于性质的测试的 Python 库。基于性质的测试根据给定的某些性质来验证函数（或模块，甚至系统）的正确性。它使用大量自动生成的测试用例，而不是单个测试用例，来有效地对函数进行压力测试。如果发现某些性质被违反，它会缩小并返回反例输入。

在下面的代码中，我们展示了如何使用经典的*基于示例的测试*来测试一个 `sort` 函数（参见 `test_sort_by_example`），以及如何使用*基于性质的测试*（基于 Hypothesis，参见 `test_sort_by_property`）来测试这个 `sort` 函数。这里，`sort` 函数将数字按升序排序。
...
> 在内部，此策略使用 `insert` 操作来添加带有（键，值）的节点并构建 BST。
...
为了让你熟悉 Pytest，你可以在 `lab4/tests` 目录下运行 `lab4/tests/simple_test.py`，该测试测试了一个有缺陷的 BST 版本（对应于 `lab4/bugs/bug1.py`）：
...
你应该获得以下测试结果，并能在 `lab4/tests/report/test1` 中找到缩小的测试用例：
...
你应该获得以下测试结果，并能在 `lab4/tests/report/test2` 中找到缩小的测试用例：
...
蜕变测试是在许多上下文中解决 oracle 问题的一种成功方法。基本思想是：即使预测函数调用（如 `insert(key, value)`）的预期结果可能很困难，我们仍然可以表达这个结果与相关调用的结果之间的预期关系。例如，如果在调用 `insert(key, value)` 之前向 BST 中插入一个额外的键，我们期望该额外的键也出现在最终的 BST 中。我们将这种蜕变关系形式化为以下蜕变性质：

```python
# 在 (key1, value1) 和 (key2, value2) 上执行两次 insert 操作与其预期结果之间的蜕变关系
# 如果键相同，则插入 value2（而不是 value1）；否则，同时插入 value1 和 value2。
# 等效函数，用于判断两个 BST 在包含的（键，值）对方面是否等效，同时忽略树结构之间的差异。
...
你可能想知道为什么我们需要检查 key1 和 key2 是否相同。原因是 `insert` 操作遵循*最后插入者获胜*的原则。因此，以下蜕变关系是有缺陷的，可能导致测试中的误报。
...
(1) 在本节中，你需要在 `lab4/src/BSTUtils.py` 中定义 `equivalent` 函数，该函数检查两个 BST 在包含的（键，值）对方面是否等效，同时忽略树结构之间的差异。你可以在代码注释中找到一些提示。
...
之后，你可以运行以下命令来确认你的性质是否能成功找到 `lab4/bugs/bug3.py` 中植入的一个错误。请注意，我们只在 `delete` 中植入了一个错误，`union` 是正确的，没有错误。如果你的性质在 `union` 中发现了错误，你可能需要仔细检查你的性质定义是否正确。
...
你应该获得以下结果，并能在 `lab4/tests/report/test3` 中找到缩小的测试用例：
...
按照上面关于 `insert` 的示例，你需要分别为操作 `delete` 和 `union` 定义一些基于模型的性质，以识别 `lab4/bugs/bug4.py` 中植入的两个错误：
...
你应该获得以下结果，并能在 `lab4/tests/report/test4` 中找到缩小的测试用例：
...
*注意：我们将验证你提交的代码是否针对错误。如果你的代码是针对错误的，你的分数可能会被相应扣除。*
...
在本实验中，你将构建一个 delta debugger，实现一种高效算法，在给定一个较大的崩溃输入后，找到 1-minimal 的崩溃输入。你将把这个工具与一个模糊测试器（比如你在 `lab3` 中构建的那个）结合起来，以最小化模糊测试器找到的崩溃输入。

## 环境搭建

Lab 4 的代码位于 `/lab5/` 目录下。

本实验建立在之前实验的基础上。我们为你提供了预编译的二进制文件，包括 `runtime` 库、用于 `coverage` 和 `sanitize` 的 `InstrumentPass`，以及一个 `fuzzer` 可执行文件；你可以在 `lab5/lib` 目录下找到它们。它们的实现与 `lab3` 中的实现完全相同。
...
与 `c++` 不同，修改代码后你不需要重新运行此命令。此外，你将能够在终端中使用 `delta-debugger` 命令来使用你的 delta debugger。

`delta-debugger` 工具执行 delta 调试，以缩小导致程序崩溃的输入。

### 步骤 2

要将 `delta-debugger` 与某个程序一起使用，你首先需要找到一些会导致该程序崩溃的输入。为了找到这样的输入，我们将使用一个模糊测试器。

和 lab3 一样，要运行 `fuzzer`，你首先需要对程序进行插桩，并设置合适的输出目录，模糊测试器会将结果存储在这些目录中。

```sh
/lab5/test$ make sanity1               # 对 sanity1 进行插桩并构建。
...
最后一个参数是崩溃输入的路径，具体取决于你想要最小化哪个输入。在这个例子中，精简后的输入存储在 `fuzz_output/failure/input1.delta` 中。此外，在再次调用 `delta-debugger` 之前，请确保清理 `fuzz_output` 目录。
...
你需要编辑 `lab5/delta_debugger/delta.py` 文件来构建一个 delta 调试工具。我们为你提供了一个模板函数——`delta_debug`——用于实现你的最小化逻辑。`delta_debug` 函数接收一个 `target` 程序，以及一个导致 `target` 崩溃的 `input`，并且应该返回一个仍然会导致 `target` 程序崩溃的 1-minimal 输入。

为了执行 delta 调试，你需要反复使用不同的 `input` 字符串来运行 `target`。我们提供了一个 `run_target` 函数来帮助你使用 `input` 运行 `target` 程序。如果目标程序没有崩溃，它会返回 0。
...
你可能想要添加一个辅助函数，例如命名为 `_delta_debug`，它接收一个 `target`、一个 `input` 和一个参数 `n`（对应搜索粒度），并执行 delta 调试算法的一次迭代，以返回下一个 `input` 和 `n`。

## 示例输入和输出

你的 delta debugger 应该能够在任何接受来自 `stdin` 输入的可执行文件上运行。

你通过传入以下参数来在测试程序上运行 delta debugger：
...
你将通过编写一个 LLVM pass 来实现这一点。

由于为像 C 这样的完整语言开发静态分析器并非易事，本实验将分为两部分。
...
4. 实现 `join` 函数，考虑 Domain 值，对两个 Memory 对象取并集。
5. 实现 `equal` 函数，考虑 Domain 值，检查两个 Memory 对象是否相等。
...
`opt` 命令优化该 LLVM IR 程序，并生成一个等效的 LLVM IR 程序，使其更易于你将在本实验中构建的分析器处理。
...
**如果你还没有完成代码，运行以下命令会导致段错误，这是正常的。完成代码后再重新运行测试。**

然后，你将使用 `opt` 命令在优化后的 LLVM IR 程序上运行这个 pass，如下所示：
...
* 程序可以有赋值、有符号和无符号算术运算（+、-、*、/）以及比较运算（<、<=、>、>=、==、!=）。
  所有其他指令都被视为 nop。
* 程序可以有 if 语句和循环。
* 用户输入仅通过提供的 `isInput` 函数返回 `True` 的函数集引入。
  你可以忽略对其他函数的调用指令。
...
更具体地说，你的任务是实现分析如何从提供的抽象域（定义在 `Domain.h` 中）中的抽象值上评估不同的 LLVM IR 指令。

在实验的第二部分，我们将专注于实现第3项，合并单个转移函数的结果，以获得一个过程内、流敏感、路径不敏感的除零分析。
...
通过阅读 [A Menagerie of Program Abstractions][Menagerie Link] 这篇文章，重新理解程序抽象的概念。

当你对抽象域有了很好的理解后，研究 `Domain` 类，以理解我们为你定义在本实验中使用的抽象域。
文件 `include/Domain.h` 和 `src/Domain.cpp` 包含了抽象值及其上的操作。
这些操作将执行抽象评估，**而无需运行程序**。
正如文章中所述，我们为加法、减法、乘法和除法定义了抽象运算符。

这个分析的一个重要部分是认识到你实际上从未运行程序。
这意味着当你去评估像这样的指令时：
...
所以，更具体地说，如果 `%x` 的 Domain 是 `Domain::Zero`，`%y` 的 Domain 也是 `Domain::Zero`，由于小于比较会被认为是 **[相等时为假][LLVM CmpInst]**，那么结果 Domain 将是 `Domain::Zero`。

#### 第二步

检查 `DivZeroAnalysis::runOnFunction` 以理解编译器 pass 在高层级上如何执行分析：
```cpp
...
```
`runOnFunction` 过程在编译器在 pass 期间遇到的输入 C 程序中的每个函数上被调用。
...
一旦 **In** 和 **Out** 映射被初始化，`runOnFunction` 就会调用 `doAnalysis`：一个你将在第二部分中实现的函数，用于执行混沌迭代算法。
对于第一部分，你可以假设它只是使用适当的 `InMap` 和 `OutMap` 映射来调用 `transfer`。

所以，在高层级上，`runOnFunction` 将：
1. 初始化 **In** 和 **Out** 映射。
2. 使用混沌迭代算法填充它们。
...
由于我们将变量称为 `std::string`，我们提供了一个名为 `variable` 的辅助函数，它将 LLVM `Value` 编码为我们用于变量的内部字符串表示。
...
请注意，在 LLVM 框架中，赋值指令（例如，call、二元运算符、icmp 等）的对象也代表它定义的变量（即它的左侧）。
...
此时，你的 `eval(...)` 实现将接收指令并确定该指令的 Domain 如何受操作影响。
例如，
...
如果源代码中的一个变量有多个赋值，这些赋值在 LLVM IR 中被拆分为不同的变量，然后**合并**在一起。
我们称这个合并点为 **phi 节点**。
...
`%x` 在分支执行后通过 `phi` 指令赋值；抽象地说，`phi i32 [ %inc, %then ], [ %dec, %else ]` 表示如果走了 then 分支，则将 `%inc` 赋值给 `%x`，如果走了 else 分支，则将 `%dec` 赋值给 `%x`。

这里有一段示例代码可以帮助你处理 phi 节点，因为具体细节超出了本课程的范围；不过，如果你对这些编译器细节感兴趣，可以随意阅读更多关于 SSA 的内容。
...
正如我们在设置部分演示的那样，使用 `opt` 在你的测试文件上运行分析器：
...
现在你已经有了填充 in 和 out 映射并使用它们检查除零错误的代码，下一步是在 `src/ChaoticIteration.cpp` 的 `doAnalysis` 函数中实现混沌迭代算法。
...
以下是一个示例，说明如何将指令加载到 `WorkSet` 中，以及如何引入 [llvm::SetVector][LLVM SetVector] 容器，你可以随意使用此代码作为实现的一部分：
...
在 `flowIn` 中，你将执行到达定值分析的第一步，即取 `I` 所有前驱的 **OUT** 变量的并集。
...
为了合并两个内存状态，你需要实现下面模板化的 join 函数：

* `Memory* join (Memory *M1, Memory *M2)`

在这个函数中，你还需要在合并这些 `Memory` 对象时考虑 `Domain` 值。
请参考抽象域以了解为什么这是必要的。
...
如果在应用 `transfer` 后内存值存在变化，你将需要将指令 `I` 提交进行额外的分析。
为了确定在 `transfer` 函数期间内存是否发生了变化，你将实现 `equal` 函数：

* `bool equal(Memory *M1, Memory * M2)`

在这个函数中，你需要在确定两个 `Memory` 对象是否相等时再次考虑 `Domain` 值。
...
完成上述步骤后，你的分析应该生成两个输出文件。
1. `test.out`，其中 test 是你正在测试的程序，是结果的精简版本，仅包含具有潜在除零操作的指令。
2. `test.err` 是一个完整的报告，包括任何具有潜在除零操作的指令以及每个被审查指令的 `InMap` 和 `OutMap` 的最终状态。
...
编写一个针对 C 程序的“除零”静态分析，作为处理指针别名和动态分配内存的 LLVM pass。
...
本实验建立在实验 5 的工作基础之上，因此你可以复用 `/lab5/src` 目录中的大部分内容。
...
第一行 (`clang`) 从输入的 C 程序 `test13.c` 生成 LLVM IR 代码。
...
* 你*不必*精确处理 XOR、OR、AND 和 Shift 运算，但你的程序在这些情况下不得引发段错误。
* 输入程序*可以*包含 if 语句和循环。
...
在本实验中，你将扩展在实验 6 中实现的**除零**分析，以分析和捕获存在别名内存位置时的潜在**除零**错误。

在课堂上，你了解到在语言中引入别名会使推理程序行为变得更加困难，
并且需要某种形式的指针分析。
你将使用一种**流不敏感的指针分析**
——我们从中抽象出控制流并构建一个全局的**指向图**
——来帮助你的检查器分析更有意义的程序。
...
函数 `f()` 的签名中没有参数。
实际上，函数可以接受任意数量的变量，
甚至可以是不同类型的变量（但本实验中，将所有参数视为 `int`）。
...
给定一个传入 `doAnalysis()` 例程的任意函数 `F`，找到该函数调用的参数，并为每个参数实例化抽象域值。
...
请确保在复用上一个作业的代码时，复制你的实现细节和函数内容，但**保持函数签名不变！**。

这些参数在我们探索指针别名时是必需的。

为了帮助理解代码与实验 6 的不同之处以及它们是如何结合在一起的，
请考虑以下来自 `DivZeroAnalysis::runOnFunction()` 的片段：
...
以及以下来自 `DivZeroAnalysis::doAnalysis()` 的片段：
...
这里我们提供了一个在 LLVM 中使用指针的接口。

你可以按原样使用它作为后备方案，但也可以自由地按照自己的方式建模 LLVM 中的引用。

对于本实验，我们禁用了实验 6 中使用的 `mem2reg` pass。
因此，LLVM 将为每个 C 变量创建一个内存单元。
结果，你将看不到任何 **phi 节点**，并且不一定需要
你在实验 6 中为处理它们而实现的代码段。
...
为了帮助建模对应于变量 `%a`（即 `variable(I1)`）的内存位置，
我们提供了一个函数 `address`，
...
这应该通过在 `transfer` 函数中添加代码来处理 `StoreInst` 和 `LoadInst` 指令来完成。
...
这将 `%d` 处指针的值（它本身是一个指针）加载到类型为 `i32*` 的新变量 `%1` 中。

**注意** 与前面的示例相比，load 指令的类型（`load i32*`）中多了一个 `*` 字符。
你可以使用 `getType()` 检索此 load 指令的类型，
并使用诸如 `isIntegerTy()` 或 `isPointerTy()` 之类的方法进一步检查类型。
...
这确保了所有指针引用都是同步的，并且会在我们的分析中收敛到一个精确的抽象值。
...
在本实验中，你将实现一个动态符号执行（DSE）引擎，它能自动生成输入，高效地探索不同的程序路径。
...
3. 在 `src/Strategy.cpp` 中实现一个回溯搜索算法，用于探索新的程序路径。

## 环境配置

Lab 7 的骨架代码位于 `/lab7` 目录下。
在描述文件位置时，我们会经常将 Lab 7 的顶层目录称为 `lab7`。
...
现在你应该能在当前目录（lab7/build）下看到 `dse` 和 `InstrumentPass.so` 这两个文件。
...
初始状态下，你会看到 `formula.smt2` 未找到的错误，因为你还没有实现插桩部分。
...
DSE 同时跟踪运行时值和符号约束，并在程序计算树上的回溯搜索过程中，利用前者来简化对后者的求解。
...
你可能会使用循环来检查数字，或者找一个库来处理矩阵乘法。
这是因为大多数编程语言都是命令式的，这意味着需要一系列命令来解决问题。

另一方面，Z3 有一个声明式接口，在这种情况下，这意味着你只需要给它提供约束列表（在这个例子中是 `X < Y` 和 `X > 2`）。
...
这个动态符号执行实现的第一个组件是对输入程序的插桩，这在 `src/DSEInstrument.cpp` 中完成。
这遵循了之前实验中熟悉的格式和模式，但现在这个 LLVM pass 将注入 `src/Runtime.cpp` 中定义的各种函数，并附带来自每个有效 LLVM 指令的适当元数据。
...
在下面的示例代码中，动态符号执行引擎会将变量 `x` 和 `y` 视为符号输入，而 `z` 则具有具体值 0：
...
请注意，`DSE_Input` 是一个宏，会展开成一个唯一的 ID。
详情请参见 `include/Runtime.h` 和 `src/SymbolicInterpreter.cpp`。

初始状态下，DSE 引擎会为输入变量分配随机数。
...
你首先需要对输入程序进行插桩，使其在 main 函数的开头调用 `__DSE_Init__` 函数。
...
接下来，你需要对其余的 IR 指令进行插桩。
通常，如果指令中的某个操作数会改变符号内存状态，那么该操作数就应该被插桩。
...
你需要为每个 LLVM 指令定义符号操作函数，并对输入程序进行插桩，使其在运行时调用这些函数。
在程序实际执行的同时，DSE 引擎会操作一个符号内存状态。
...
具体执行的符号操作使用两个辅助函数 `__DSE_Const__` 和 `__DSE_Register__` 执行，每个函数将具体常量和寄存器编码为它们的符号对应物。
这些函数在 `src/SymbolicInterpreter.cpp` 中定义。
函数 `__DSE_Const__` 接收一个 LLVM IR 的常量整数，为该数字创建一个符号表达式，并将该符号表达式压入栈（类 `SymbolicInterpreter` 中的 `Stack` 字段）。
...
像 `llvm::Inst::CmpInst` 和 `llvm::BinaryOperator` 这样的指令会操作符号，并且需要在约束中进行等效表示。
你将使用 Z3 表达式来表示这些操作。
Z3 API 使用了 C++ 的一个称为[运算符重载](https://en.cppreference.com/w/cpp/language/operators)的特性，允许你对 `z3::expr` 类型的对象使用 C++ 算术和比较运算符。
下面我们展示一些示例，说明如何在 `z3::expr` 对象上表示算术和比较表达式。
...
回顾“示例 1：组合方法”讲座视频中，为了能让 DSE 分析探索输入测试程序的更多路径，条件是如何被处理的。
修改 `src/Strategy.cpp` 中的 `searchStrategy()` 函数以执行这种回溯行为。
它应该修改当前将提供给 Z3 的路径公式，以便 Z3 能推导出新的输入。

#### 路径公式与搜索策略

在每次执行插桩程序后，路径公式将被编码并存储在 `formula.smt2` 中。
所有已执行分支指令的 ID 将按执行顺序存储在 `branch.txt` 中，这可能有助于生成下一个输入。
给定当前可满足的路径公式，`searchStrategy` 函数将提出一个公式，用于推导能够探索更多路径的新输入。
