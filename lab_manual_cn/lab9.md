...
- [动态符号执行](#dynamic-symbolic-execution)
   ...
   * [4. 任务 1 实现符号化过程](#4-任务-1-实现符号化过程)
      + [4.1 你需要了解的函数](#41-你需要了解的函数)
      + [4.2 提示](#42-提示)
      + [4.3 概念验证 (POC)](#43-概念验证-poc)
   * [5. 任务 2 解释 `Add` 和 `Sub` 的语义](#5-任务-2-解释-add-和-sub-的语义)
      + [5.1 你需要了解的函数](#51-你需要了解的函数)
      + [5.2 提示](#52-提示)
      + [5.3 概念验证 (POC)](#53-概念验证-poc)
   * [6. 任务 3 解释 `Br` 的语义](#6-任务-3-解释-br-的语义)
...
> 由于 Windows 和我们较旧的 Docker 镜像的虚拟化机制不同，请注意以下调整

**如果您使用 Windows，请改用 WSL2。**
...
```
打开 WSL2，下载您的仓库，并使用 VSCode（使用 Windows 版本的 VSCode，无需单独下载）打开它：
...
此外，一个潜在的问题是当前容器中的 LLVM 版本已过时，不适合运行 MiniKLEE。为了解决这个问题，我添加了一个热补丁，以便在容器启动时**自动**从网络下载合适的 LLVM 版本。但是，对于**网络状况不佳**的同学，最好切换到国内镜像，例如阿里云或清华镜像。切换镜像后，重新下载相应的 LLVM 版本。您可以重启容器以**自动**下载，也可以**手动下载**：

``` bash
lab8$ rm llvm.sh                        # 移除当前的 llvm.sh
lab8$ wget https://apt.llvm.org/llvm.sh # 从网络下载 llvm.sh
lab8$ chmod +x llvm.sh                  # 授予执行权限
...
在本实验中，您将实现一个动态符号执行 (DSE) 引擎 `miniklee`，包括：

1. 实现符号化过程
2. 实现一个执行生成测试 (EGT) 符号执行框架。
   1. 解释 `Add` 和 `Sub` 的语义
   2. 解释 `Br` 的语义
...
<!-- TOC --><a name="21-动机示例"></a>
### 2.1 动机示例

我们提供了可执行文件 `refminiklee`（位于 `./`），以便快速了解符号执行。

我们的动机示例如下。假设 `__builtin_trap()`（第 9 行）是假定的 bug。程序首先将变量 `a` 设为符号变量。然后，它遇到一个 `if` 分支 (`if (a + 2 == 100 - 10)`)。当 `a` 的值设置为 88 时，可以触发假定的 bug。
...
`refminiklee` 的工作方式如下：
1. 它将（第 5 行）变量 `a` 更新为虚拟机内存中的符号变量。
2. 接着，它检查 if 分支（第 7 行），并将分支语句编码为约束 (`a + 2 == 100 - 10`)，然后将其与之前的路径约束（此示例中无）一起发送给约束求解器。
3. 然后，求解器确定约束的可行性，结果为 `真分支和假分支都可行`。
4. `miniklee` 然后分叉状态，并将新状态更新到状态池。
5. 对于通过真分支的状态，`miniklee` 生成一个测试用例（带有 `error` 后缀），其中 `a = 88`；对于假分支，它生成一个测试用例（带有 `normal` 后缀），其中包含一个不等于 88 的随机值。
...
为简化起见，本实验中的输入程序假定**仅具有 C 语言的以下子特性**：
- 所有值都是 32 位有符号整数（即没有浮点数、指针、结构体、枚举、数组等）
- 待测程序仅包含 main 函数，这意味着没有函数调用
- 涉及符号变量的条件要么是相等，要么是不相等
- 仅涉及以下操作：
    ...
    - [Call](https://llvm.org/doxygen/classllvm_1_1CallInst.html)：用于抽象目标机器调用约定的指令
...
<!-- TOC --><a name="23-环境搭建"></a>
### 2.3 环境搭建

要构建您自己的 `miniklee`，只需在 `lab8` 目录下输入 `make`：

``` bash
$ make
```

然后您将在当前目录中获得当前版本的 `miniklee`，它目前只能测试简单的测试用例（例如 `test/sequential.c`）：
...
您可以在终端日志中观察到生成了一个测试用例（例如 `-2082133583`，一个随机生成的值）。但是，`result_1/test_case_1.error` 中没有分配的值，因为 `sequential.c` 中没有符号变量。换句话说，`sequential.c` 没有接收任何输入。

您的第一个任务是使变量成为符号变量（在第 4 节中）。
...
准备工作之后，`miniklee` 在函数 `runFunctionAsMain` 中执行符号执行：
...
2. 在主循环中，如果状态池不为空，则继续：
   ...
   2. 获取要执行的下一条指令。
   3. 执行指令并进行分析。
   4. 通过添加新状态和移除死亡状态来更新状态。
3. 当一个状态到达终止点时，它会为该状态生成一个测试用例。

- `src/Executor.cpp:executeInstruction`

专注于指令执行，这部分主要涉及一个大型的 `switch-case` 语句。它识别并解释获取到的指令。例如：
- 解释 `Ret` 指令会终止当前状态。
...
我们可以在待测程序中使用它，如下面的代码片段所示。实际上，KLEE 符号执行器也使用这种方法将变量标记为符号变量（KLEE 官方文档 [将输入标记为符号变量](https://klee-se.org/tutorials/testing-function/)）。
 ...
 **提示 2 深入理解程序的表示**
 ...
 我们从不同的角度表示程序，例如：
 - **源代码**
   这是程序的具体语法，其优点是易于阅读，但难以转换、分析和优化。下面是一个计算存储在 `value` 中的数字（本例中为 `8`）的阶乘的示例。
 ``` c++
 int value = 8;
 int result = 1;
 ...
 ```
   ...
   [抽象语法树](https://en.wikipedia.org/wiki/Abstract_syntax_tree) (AST) 表示有利于编译器，因为它简化了解析和分析。对于解释器来说，它特别有用——递归处理树可以通过解释每个语句并更新程序环境来轻松评估块。AST 的主要缺点是其节点类型的多样化行为。编写编译器分析需要不断处理这些节点类型之间的语义差异，这可能很麻烦。存在更适合实现复杂编译器优化的替代表示。这些方法使表示更加规则，从而简化了过程。

 - **指令**（特别是 LLVM IR）
   LLVM IR 是静态[单赋值](https://en.wikipedia.org/wiki/Static_single_assignment_form) (SSA) 形式，这在 [LLVM IR 讲座](https://tingsu.github.io/files/courses/slides/lec-2-llvm-framework-primer.pdf) 中已经解释过。我们喜欢指令表示，因为它具有规则性。然而，要利用它做任何有用的事情，我们需要提取更高级别的表示（**`miniklee` 是在基本块和指令级别上工作的**）：
   ...
   我们欣赏 LLVM IR，特别是它的 SSA（静态单赋值）特性：每个变量在全局范围内都有一个静态赋值。然而，这并不意味着动态单赋值，因为同一个语句可以执行多次。总之，在 LLVM IR 代码中：
   - 定义 == 变量
   - 指令 == 值
   - 参数 == 数据流图边
...
void Executor::executeMakeSymbolic(ExecutionState& state,
                                   ...
                                   std::string name) {
    // 将要符号化的变量（Instruction 类型）注册到符号内存
...
实现任务 1 后，运行测试用例 `test/symbolic.c` 以验证您的更改。此测试引入了符号变量，允许您观察符号执行引擎如何处理它们。检查生成的测试输入，确保符号输入被识别并正确处理。
...
Test case written successfully (result_1/test_case_1.error)
```
现在您可以看到 `result_1/test_case_1.error` 中为符号值 `a`（假定为程序输入）分配了一个值（此处 `-399613364` 是随机生成的）。

<!-- TOC --><a name="5-任务-2-解释-add-和-sub-的语义"></a>
...
实现任务 2 后，运行测试用例 `test/example.c` 以验证您的更改。此测试引入了 `add` 操作、`sub` 操作和分支条件，允许您观察 `miniklee` 如何处理它们。
...
<!-- TOC --><a name="6-任务-3-解释-br-的语义"></a>
## 6. 任务 3 解释 `Br` 的语义

**为 `Br` 实现语义解释 (`src/Executor.cpp:executeInstruction` 和 `src/Executor.cpp:fork`)。我们提供了标记为“Task 3: Your Code Here”的代码骨架。**

分支处理是执行生成测试 (EGT) 符号执行器的核心。在继续之前，让我们回顾一下[其背景](https://dl.acm.org/doi/10.1145/2408776.2408795)。

目前有两种类型的符号执行：

- **Concolic 测试**：使用随机输入开始执行，执行后，使用当前路径的路径条件 pc₀（通过否定最后一个条件）构建一个新的路径条件 pc₁，并求解 pc₁ 以获得新的输入 I₁ 来探索新路径，并重复该过程。如 [CREST](https://www.burn.im/crest/) 和 [DART](https://dl.acm.org/doi/10.1145/1064978.1065036) 中所示。
_我们之前的实验属于这种类型，[MIT 6.858: 计算机系统安全 实验 3: 符号执行](https://css.csail.mit.edu/6.858/2022/labs/lab3.html) 也是如此。_

- **执行生成测试 (EGT)**：在每个条件分支处分叉符号执行（如果两个方向都可行），维护多个部分路径，并同时协调它们的执行。如 [EXE](https://dl.acm.org/doi/10.1145/1455518.1455522)、[SPF](https://dl.acm.org/doi/10.1145/1858996.1859035) 和 [KLEE](https://dl.acm.org/doi/10.5555/1855741.1855756) 中所示。
_这是 `miniklee` 实现的类型。_

如上所述，EGT 执行器在每个条件分支处，如果两个方向都可行，则分叉其执行。您的任务是实现用于解释 `Br` 指令的 `fork` 函数。
 ...
 **提示 1：理解在 `miniklee` 中执行 `Br` 的工作流程**

 当执行器处理 `Br` 指令时，它首先将其转换为相应的 `Br` 类型（[讲座：LLVM 框架，第 37 页](https://tingsu.github.io/files/courses/slides/lec-2-llvm-framework-primer.pdf)）。然后它使用 API（[BranchInstr 的 isUnconditional](https://llvm.org/doxygen/classllvm_1_1BranchInst.html#a7e4be8b16fbd68c9045a388904044e01)）来确定分支是无条件还是条件分支，并分别处理每种情况：

 - **无条件**：将执行状态转移到下一个基本块。
 - **条件**：提取条件并将其发送给求解器以检查可行性：
   - **真分支可行**：转移到真分支的基本块。
   - **假分支可行**：转移到假分支的基本块。
   - **两个分支都可行**：为每个分支分叉新状态，将控制权转移到它们各自的基本块，并将分叉的状态添加到状态池。

 **提示 2：理解 `fork` 函数的工作流程**

 为了减轻实验的痛苦，我们已经为 `fork` 函数 (`src/Executor.cpp:fork`) 提供了一个骨架，`fork` 的工作方式如下：

 - **调用求解器确定分支的可行性**：它首先调用求解器，检查在给定查询（路径约束下的真分支和假分支）下是否存在使条件评估为真的有效赋值。明确地说，如果真分支可行，我们将 `trueBranch` 设置为 true；否则为 false。类似地，如果假分支可行，`falseBranch` 设置为 true；否则为 false。您可以按如下方式实现此部分：
...
### 6.3 概念验证 (POC)
实现任务 3 后，运行测试用例 `test/example.c` 以验证您的更改。您现在应该看到与运行 `refminiklee` 相同的结果（除了随机生成的值）。请根据 [2.1 动机示例](#21-动机示例) 进行自我检查。

<!-- TOC --><a name="7-提交"></a>
## 7. 提交

完成实验后，通过提交并推送 lab8/ 下的更改来提交您的代码。具体来说，您需要提交对 `src/Executor.cpp` 的更改。

``` bash
   lab8$ git add src/Executor.cpp
   lab8$ git commit -m "您的提交信息"
   lab8$ git push
```