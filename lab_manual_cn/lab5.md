...
与 `c++` 不同，你在修改代码后无需重新运行此命令。
此外，你可以通过终端中的 `delta-debugger` 命令来使用你的 delta debugger。

`delta-debugger` 工具执行 delta debugging，以将程序的崩溃输入缩小。
...
与 lab3 一样，要运行 `fuzzer`，你首先需要插桩该程序，并设置适当的输出目录，以便模糊测试器存储其结果。
...
运行模糊测试器后，你将在 `test/fuzz_output_sanity1/failure` 目录下找到导致程序崩溃的输入。
...
最后一个参数是崩溃输入的路径，取决于你想要最小化哪个输入。
在此示例中，缩减后的输入存储在 `fuzz_output/failure/input1.delta` 中。
...
`delta_debug` 函数接收一个 `target` 程序，以及一个导致 `target` 崩溃的 `input`，并应返回一个仍然会导致 `target` 程序崩溃的 1-minimal 输入。

为了执行 delta debugging，你需要使用不同的 `input` 字符串反复运行 `target`。
...
你可能希望添加一个辅助函数，例如命名为 `_delta_debug`，它接收一个 `target`、一个 `input` 和一个参数 `n`（对应于搜索粒度），并执行 delta debugging 算法的一次迭代，以返回下一个 `input` 和 `n`。

### 示例输入和输出

你的 delta debugger 应该能够在任何接受来自 `stdin` 输入的可执行文件上运行。

你可以通过传递以下参数来在测试程序上运行 delta debugger：
...
作为一个具体示例，考虑字符串："This is theh) "，它会导致 `test2` 失败：
